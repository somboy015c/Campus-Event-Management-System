...
class Home_controller extends Home_Core_Controller
{
	...
	public function index()
	{
		...
		if (!auth_check()) {
			...
			redirect(lang_base_url().'members');
			$this->load->view('partials/_footer');
		}
	}
...
/**
	...
	public function products()
	{
		...
		if ($data['search_type'] == "product"){if ($search == 'promoted_products') {$data['heading'] = 'Promoted Products';} elseif ($search == 'product_for_sales') {$data['heading'] = 'Products For Sale';} elseif ($search == 'top_products') {$data['heading'] = 'Top Products';} elseif ($search == 'trending_products') {$data['heading'] = 'Trending Products';} elseif ($search == 'latest_products') {$data['heading'] = 'Latest Products';} else {$data['heading'] = 'Products';}} elseif ($data['search_type'] == "Handwork") {if ($search == 'promoted_works') {$data['heading'] = 'Promoted Handworks';} elseif ($search == 'top_works') {$data['heading'] = 'Top Handworks';} elseif ($search == 'trending_works') {$data['heading'] = 'Trending Handworks';} elseif ($search == 'latest_works') {$data['heading'] = 'Latest Handworks';} else {$data['heading'] = 'Handworks';}}

		$data['title'] = ($data['heading']);
		$data['description'] = ($data['heading']) . " - " . $this->app_name;
		$data['keywords'] = ($data['heading']) . "," . $this->app_name;
		//get paginated posts
		if ($data['search_type'] != "product" && $data['search_type'] != "Handwork") {
			redirect(lang_base_url());
		}
		$link = lang_base_url() . 'products';
		if ($data['search_type'] == "product"){
			if ($search == 'promoted_products') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_promoted_products_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_promoted_products(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'product_for_sales') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_products_forsale_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_forsales(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'top_products') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_top_products_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_top_products(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'trending_products') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_trending_products_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_trending_products(null, $pagination['per_page'], $pagination['offset']);
		...
		} elseif ($data['search_type'] == "Handwork") {
			if ($search == 'promoted_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_promoted_works_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_promoted_works(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'top_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_top_works_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_top_works(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'trending_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_trending_works_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_trending_works(null, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'latest_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_latest_works_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_latest_works(null, $pagination['per_page'], $pagination['offset']);
			} else {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_works_count(null), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_works(null, $pagination['per_page'], $pagination['offset']);
			}
		}
	...
	public function category($category)
	{
		...
		if ($data['search_type'] == "product"){
			if ($search == 'promoted_products') {
				...
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_products_forsale_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_forsales($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'top_products') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_top_products_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_top_products($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'trending_products') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_trending_products_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_trending_products($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'latest_products') {
			...
			if ($search == 'promoted_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_promoted_works_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_promoted_works($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'top_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_top_works_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_top_works($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'trending_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_trending_works_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_trending_works($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} elseif ($search == 'latest_works') {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_latest_works_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_latest_works($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			} else {
				$pagination = $this->paginate($link, $this->product_model->get_paginated_filtered_works_count($data["category"]->id), $this->product_paginate_per_page);
				$data['products'] = $this->product_model->get_paginated_filtered_works($data["category"]->id, $pagination['per_page'], $pagination['offset']);
			}
		} else {
	...
	public function members()
	{
		...
		get_method();
		$data['title'] = ('Events');
		$data['description'] = ('Events') . " - " . $this->app_name;
		$data['keywords'] = ('Events') . "," . $this->app_name;
		$data['tab'] = "events";
		$data["categories"] = $this->category_model->get_parent_categories();
