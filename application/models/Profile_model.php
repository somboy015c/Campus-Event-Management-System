...
class Profile_model extends CI_Model
{
    ...
    {
        $data = array(
            'skill' => $this->input->post('add_skiil', true),
            'user_id' => user()->id
        );
        return $this->db->insert('user_cv', $data);
    }
    ...
    //delete skill
    public function delete_user_skill($skill_id)
    {
        $skill_id = $this->input->post('delete_skill', true);
        $data = array(
                'is_deleted' => 1
            );
        $this->db->where('id', $skill_id);
        return $this->db->update('user_cv', $data);
        return false;
    }
    ...
    public function get_paginated_filtered_stores($category_id, $per_page, $offset)
    {
        $this->filter_members($category_id);
        $this->db->where('users.role !=', 'admin');
    ...
    public function build_query()
    {
        ...
        //default location
            if ($this->default_location_id != 0) {
                if ($this->default_state_id == 0) {
                    if ($ff > 0) {
                        $this->db->where('users.country_id', $this->default_location_id);
                    }else{
                        $this->db->where('users.state_id', $this->default_location_id);
                    }
                } else {
                    if ($ff > 0) {
                        $this->db->where('users.state_id', $this->default_state_id);
                    }else{
                        $this->db->where('users.country_id', $this->default_state_id);
                    }
                }
